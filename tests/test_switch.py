...
def _get_switch_entity_id(
    ...
    """Look up the auto_renew switch entity_id."""
    for entity in er.async_entries_for_config_entry(entity_registry, entry_id):
        if entity.unique_id.endswith("_auto_renew"):
            return entity.entity_id
    raise AssertionError("No auto_renew switch entity found")
...
async def test_switch_shares_device(
    ...
    """Test the switch is under the same device as sensors."""
    devices = dr.async_entries_for_config_entry(
        device_registry, setup_integration.entry_id
    )
    # All entities (sensors + switch) should share a single device
